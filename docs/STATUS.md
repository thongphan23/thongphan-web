...
- R0.1 design is corrected and split into local remediation and owner-gated production cutover. R0.1A Tasks 1–5 are complete in local source; Tasks 6–8 remain pending. R0.1B production cutover has not started.
...
- Latest R0.1A verification after Task 5 passes: focused signup/email/route suite `20/20`, full suite `273/273`, root and Worker TypeScript, lint, current-tree secret-integrity scan with zero findings, static build `82/82`, and a local D1 fixture with one signup row and zero queue rows.
...
- Production is unchanged by R0.1A Tasks 1–5: no deploy, route mutation, D1 write or migration, email send, audience import, cron activation, credential mutation, or Git-history rewrite occurred. R0.1B remains a separately owner-gated production action.
