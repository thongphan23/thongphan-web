...
- Reproduced the owner's report against Round 3: 24/51 shots contained a
  generated moving crop. Fifteen could already retain their semantic carrier
  with one static crop; nine mixed detector identities or source sub-shots.
- The root cause was an unsafe chain: three independently selected saliency/
  face observations became an automatic crop path, Remotion interpolated that
  path on every frame, and QA measured retention/step size without requiring an
  editorial reason for camera movement.
- Upgraded the active plugin to static-first composition. A moving crop now
  requires reviewed `camera_motion_intent`, one stable `semantic_carrier_id`
  across the shot and a same-carrier frame-crossing need. The renderer ignores
  every unreviewed path. Plugin commit `786dc52`, active version
  `0.2.10-rc.1+codex.20260801211939`; focused 10/10 and 8/8, adjacent 99/99 and
  full 809/809 tests pass.
- Rebuilt all three films as Round 4. Camera movement generated by the crop
  system fell from 24/51 to 0/51; 51/51 geometric gates and 51/51 manual
  START/MID/END stability checks pass. Mean semantic-carrier retention is
  98.35%. The three prior source-content risks remain explicit and are not
  misreported as camera failures.
- The review page now publishes 12 videos across four rounds and opens Round 4
  by default. Website focused 4/4, full 248/248, TypeScript, ESLint, static
  build and local/preview/production Playwright QA all pass; browser QA covers
  24/24 round-film combinations at desktop and mobile. Production deployment
  `f3346e78` serves implementation commit `260a679`; `/tpr` remains protected
  with HTTP 401. Rollback deployment is `8282b418`. Full evidence:
  `docs/releases/REMOTION_CAMERA_STABILITY_R4_PRODUCTION_RELEASE-2026-08-01.md`.
